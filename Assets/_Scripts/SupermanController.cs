...
public class SupermanController : MonoBehaviour {
	...
	void Update () {
        ...
        //this will cause the superman to move to right because input is positive
        if (this._playerInput > 0)
        {
            //to check bounds i.e to keep the superman in scene
            if (this._currentPosition.y < 220)
                this._currentPosition += new Vector2(0, this.speed);
            

        }
        //this will cause the supeman to move to left because input is negative
        if (this._playerInput < 0)
        {
            //to check bounds i.e to keep the superman in scene
            if (this._currentPosition.y > -220)
